...
---
flowchart TB
    A(["MULAI"]) --> B["Konsultasi ke Wali Kelas"]
    B --> C{"Masalah Bisa Diselesaikan?"}
    C -- YA --> D["Tetap Bersekolah di MTs N 1"]
    C -- TIDAK --> E["Konsultasi ke Waka Kesiswaan"]

    E --> F["Penjelasan Prosedur & Syarat"]
    F --> G["Ambil Formulir Permohonan di TU"]
    G --> H["Isi Formulir & Lengkapi Berkas"]
    H --> I["Serahkan ke TU"]
    I --> J{"Cek Kelengkapan"}
    J -- TIDAK LENGKAP --> K["Minta Dilengkapi"]
    J -- LENGKAP --> L["PROSES CLEARANCE"]

    L --> M["Cek Tunggakan: Keuangan, Perpustakaan, Perlengkapan"]
    M --> N{"Ada Tunggakan?"}
    N -- YA --> O["Lunasi/Kembalikan"]
    N -- TIDAK --> P["Surat Bebas Tanggungan"]

    P --> Q["PENYELESAIAN NILAI"]
    Q --> R["Wali Kelas Kumpulkan Nilai → Input Rapor Pindahan → Legalisir"]

    R --> S["PEMBUATAN SURAT PINDAH"]
    S --> T["Draft → Verifikasi → Cetak → TTD Kepala Madrasah"]

    T --> U["PENYERAHAN DOKUMEN"]
    U --> V["Hubungi Ortu → Ortu Ambil Berkas → TTD Bukti Terima"]

    V --> W["Dokumen Diserahkan: Surat Pindah, Rapor Pindahan, Fotokopi Buku Induk, Surat Kelakuan Baik, Surat Bebas Tanggungan"]

    W --> X["ADMINISTRASI AKHIR"]
    X --> Y["Update Buku Induk → Update Dapodik/EMIS → Laporan"]
