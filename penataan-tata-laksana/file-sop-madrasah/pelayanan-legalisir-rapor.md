...
---
flowchart TB
    A(["MULAI"]) --> B["Pemohon Datang ke Loket TU"]
    B --> C["Ambil & Isi Formulir Permohonan Legalisir"]
    C --> D["Serahkan: Formulir, Fotokopi rapor, Rapor asli, Identitas"]
    D --> E["Petugas Cek Kelengkapan Berkas"]

    E --> F{"Lengkap?"}
    F -- TIDAK --> G["Minta Dilengkapi"]
    F -- YA --> H["Verifikasi Identitas Pemohon"]

    H --> I["Cocokkan Rapor Asli dengan Fotokopi"]
    I --> J{"Sesuai?"}
    J -- TIDAK --> K["Tolak"]
    J -- YA --> L["Verifikasi Nilai dengan Leger/Arsip"]

    L --> M{"Nilai Sesuai?"}
    M -- TIDAK --> N["Konfirmasi Waka Kurikulum"]
    N --> O["Jika tetap tidak sesuai: TOLAK"]
    M -- YA --> P["Paraf Setiap Halaman"]

    P --> Q["Stempel 'Sesuai Dengan Aslinya'"]
    Q --> R["Tanggal Legalisir"]
    R --> S["Verifikasi Waka Kurikulum (Spot Check)"]
    S --> T["Disposisi ke Kepala Madrasah"]
    T --> U["TTD & Cap Kepala Madrasah"]
    U --> V["Pencatatan Buku Register"]
    V --> W["Pemberitahuan ke Pemohon (HP/WA)"]
    W --> X["Pemohon Ambil Rapor"]
    X --> Y["Cek Kesesuaian Bersama Pemohon"]
    Y --> Z["TTD Bukti Penerimaan"]
    Z --> AA["Serahkan Rapor yang Sudah Dilegalisir"]
    AA --> AB(["SELESAI"])
```
