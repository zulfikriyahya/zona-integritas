...
flowchart TB
    ...
    B --> C["Sampaikan Keperluan & Jenis Surat"]
    C --> D["Petugas Menjelaskan Prosedur & Syarat"]
    D --> E["Ambil & Isi Formulir Permohonan"]
    E --> F["Lengkapi Dokumen Persyaratan"]
    F --> G["Serahkan Formulir + Dokumen ke TU"]
    G --> H["Petugas Cek Kelengkapan"]

    H --> I{"Lengkap?"}
    I -- TIDAK --> J["Minta Dilengkapi"]
    I -- YA --> K["Verifikasi Data di Database"]

    K --> L{"Valid?"}
    L -- TIDAK --> M["Konfirmasi Ulang / Tolak"]
    L -- YA --> N["Pembuatan Draft Surat"]

    N --> O["Verifikasi Waka Kesiswaan (untuk jenis tertentu)"]
    O --> P["Cetak Surat di Kertas Berkop"]
    P --> Q["Penomoran Surat"]
    Q --> R["Disposisi ke Kepala Madrasah"]
    R --> S["TTD & Cap Kepala Madrasah"]
    S --> T["Pencatatan di Buku Ekspedisi"]
    T --> U["Pemberitahuan ke Pemohon (HP/WA)"]
    U --> V["Pemohon Ambil Surat di TU"]
    V --> W["Cek Kesesuaian Surat"]

    W --> X{"Sesuai?"}
    X -- TIDAK --> Y["Perbaikan"]
    X -- YA --> Z["TTD Bukti Penerimaan"]

    Z --> AA["Penyerahan Surat"]
    AA --> AB(["SELESAI"])
```
