...
flowchart TB
    ...
    B --> C["Mengambil & Mengisi Formulir Permohonan Legalisir"]
    C --> D["Menyerahkan: Formulir, Fotokopi dokumen, Dokumen asli (ditunjukkan), Identitas diri"]
    D --> E["Petugas TU Verifikasi Keaslian Dokumen"]
    E --> F["Pencocokan dengan Database/Buku Induk"]

    F --> G{"Dokumen Asli?"}
    G -- TIDAK --> H["TOLAK & Kembalikan"]
    G -- YA --> I["Pembubuhan Stempel 'Sesuai Dengan Aslinya'"]

    I --> J["Paraf Petugas & Tanggal Legalisir"]
    J --> K["Tandatangan Kepala Madrasah (1 hari)"]
    K --> L["Pembubuhan Cap Madrasah"]
    L --> M["Pencatatan dalam Buku Register Legalisir"]
    M --> N["Penyerahan Dokumen ke Pemohon"]
    N --> O["Pemohon Tandatangan Bukti Penerimaan"]
    O --> P(["SELESAI"])
